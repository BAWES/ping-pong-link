...
const showFieldError = (el, msg) => { if (!el) return; el.textContent = msg; el.classList.toggle('visible', !!msg); };
...
function buildEmojiGrid(gridId) {
  ...
  EMOJI_CHOICES.forEach(e => {
    ...
    b.type = 'button'; b.className = 'emoji-btn' + (e === state.me.emoji ? ' active' : '');
    b.textContent = e; b.setAttribute('aria-label', `Emoji ${e}`);
...
function buildQR(containerId, url) {
  ...
  if (!el) return;
  el.innerHTML = '';
  new QRCode(el, { text: url, width: 200, height: 200, colorDark: '#000', colorLight: '#fff', correctLevel: QRCode.CorrectLevel.M });
}
...
function setupOnboarding() {
  ...
  $('ob-next-btn').onclick = () => {
    ...
    if (!name) { showFieldError($('ob-name-error'), 'Name is required.'); $('ob-name').classList.add('input-error'); $('ob-name').focus(); return; }
  ...
  $('guest-join-btn').onclick = async () => { await enterGame(); autoClaimSeat(); };
  ...
  $('qr-continue-btn').onclick = async () => { await enterGame(); autoClaimSeat(); };
  $('qr-show-qr-btn').onclick = () => buildQR('qr-wrap', $('qr-link-input').value);

  $('ob-name').addEventListener('keydown', e => { if (e.key === 'Enter') $('ob-next-btn').click(); });
  $('ob-name').addEventListener('input', () => { showFieldError($('ob-name-error'), ''); $('ob-name').classList.remove('input-error'); });
...
async function autoClaimSeat() {
  if (!state.channel) { setTimeout(autoClaimSeat, 800); return; }
...
async function connect() {
  ...
  state.realtime.connection.on('connected', () => { state.connected = true; hideReconnect(); renderGame(); });
  state.realtime.connection.on('disconnected', () => { state.connected = false; showReconnect('Connection lost. Reconnecting…', false); renderGame(); });
  state.realtime.connection.on('suspended', () => { state.connected = false; showReconnect('Reconnection failed.', true); renderGame(); });
  state.realtime.connection.on('failed', () => { state.connected = false; showReconnect('Connection failed. Check network.', true); renderGame(); });
...
function renderPlayers() {
  ...
  state.rs.seats.forEach((seat, i) => {
    if (i === 1) { const vs = document.createElement('span'); vs.className = 'vs-badge'; vs.textContent = 'VS'; view.appendChild(vs); }
...
function renderFeed() {
  ...
  feed.innerHTML = state.rs.feed.map(item => {
    if (item.type === 'system') return `<article class="event system"><div class="event-head"><span class="event-who" style="color:var(--color-text-faint)">Room</span><span class="event-time">${timeText(item.time)}</span></div><div class="event-body">${esc(item.text)}</div></article>`;
...
async function sendTurn() {
  ...
  if (!state.channel) { showFieldError($('composerError'), 'Join a room first.'); return; }
  if (state.mySeat === null) { showFieldError($('composerError'), 'Seat not claimed yet.'); return; }
...
// ── BANNERS ───────────────────────────────────────────────────────────────
let errorTimer = null;
function showError(msg) { $('errorMsg').textContent = msg; $('errorBanner').classList.remove('hidden'); clearTimeout(errorTimer); errorTimer = setTimeout(clearError, 6000); }
function clearError() { $('errorBanner').classList.add('hidden'); }
function showReconnect(msg, btn) { $('reconnectMsg').textContent = msg; $('reconnectBanner').classList.remove('hidden'); $('reconnectBtn').classList.toggle('hidden', !btn); }
...
function boot() {
  ...
  if ($('messageInput')) {
    ...
    $('messageInput').addEventListener('keydown', e => { if (e.key === 'Enter' && (e.ctrlKey || e.metaKey)) { e.preventDefault(); sendTurn(); } });
  }
